...
DataFrames.filter(f::Function, ef::EconFrame, args...; kwargs...) = reconstruct(ef;data=DataFrames.filter(f, ef.data, args...; kwargs...))
...
DataFrames.filter!(f::Function, ef::EconFrame, args...; kwargs...) = df_function_keeping_metadata!(ef, (df, fun) -> DataFrames.filter!(fun, df), f, args...; kwargs...)
DataFrames.filter!(ef::EconFrame, args...; kwargs...) = df_function_keeping_metadata!(ef, DataFrames.filter!, args...; kwargs...)
DataFrames.filter!(p::Pair, ef::EconFrame; kwargs...) = df_function_keeping_metadata!(ef, (df, pair) -> DataFrames.filter!(pair, df), p; kwargs...)
...
function df_restore_metadata!(df::DataFrame, saved_meta)::Nothing
    for (col, key, value, style) in saved_meta
        col in names(df) && DataFrames.colmetadata!(df, col, key, value; style=style)
    end
    return nothing
end
